...
## [2026-08-31] — docs: professional README rewrite

Rewrote `README.md` as a product-facing document rather than a staged build walkthrough: results-first, architecture and design decisions as the spine, with the Stage 0 → 4 journey compressed into one "How it evolved" section. Every figure was cross-checked against this DEVLOG — 28 numeric claims, all traceable, none estimated. Added a single `docs/architecture.png` slot (with `docs/.gitkeep` so the path exists) and no other image placeholders. Verified against the code that the order route is `POST /order` (singular; `GET /orders` is a separate debug read on the engine) and that query clients hit the HTTP read API on port 3001, which reads `readmodel.*` only — they do not connect to Postgres directly. All `npm run` script names in the README were checked against `package.json`, and all 12 documented routes against the two servers.
